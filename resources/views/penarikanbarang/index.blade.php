                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><input data-id="{{$data->id}}" class="toggle-class" type="checkbox"
                                        ...
                                        data-on="Sudah Ditarik" data-off="Tidak Ditarik"
                                ...
                                <td><a href="#edit" data-toggle="modal" data-target="#formModal" data-mode="edit"
                                        ...
                                        data-tanggal="{{ $data->tanggal_expire }}" data-qty="{{ $data->qty }}"><span
...
<script>
    $(function () {
        ...
        $('#kode_barang').on('change', function (e) {
            let kode = $(this).val();
            $.ajax({
                url: "{{ route('getNamaBarang') }}?kode=" + kode,
                method: 'GET',
                success: function (response) {
                    console.log(response.barang[0].nama_barang);
                    $('#nama_barang').val(response.barang[0].nama_barang);
                    $('#barangId').val(response.barang[0].id);
                }
            });
        });
        $('#formModal').on('show.bs.modal', function (event) {
            ...
            if(mode == 'edit'){
                ...
                modal.find('.modal-body #kode_barang').val(kode);
                modal.find('.modal-body #nama_barang').val(nama);
